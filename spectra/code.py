...
def wait_for_display():
    """ Wait till the display is ready """
...
def display_init():
    """
    Full display initialization sequence as provided by the manufacturer.
    https://www.good-display.com/product/442.html (under "Download" in Sample codes)
    """
    init()
    write_cmd(0xaa)
...
def acep_color(color):
    """ Display a single 8-bit color on the display """
    write_cmd(0x10)
    for i in range(480):
        for _ in range(400):
            write_data(color)
...
def show_image_data(image_data):
    """ Display the image data on the display """
    write_cmd(0x10)
...
if __name__ == '__main__':
    ...
    folder = 'img'
    # For how long should the display sleep before changing to the next image
    sleep_time = 3600
    ...
    # Modify the image data to show the battery level
    ...
    # Sleep for `sleep_time`, then wake up and repeat the script
