# TODO: replace os.path with pathlib.Path stuff
import os
...
for filename in os.listdir(source_path):
    ...
    image_data = get_image_data(os.path.join(source_path, filename))
    colors = image_data[:, 1::2] + 16 * image_data[:, ::2]
    byte_seq = colors.tobytes()  # [::-1]

    dest_fullname = f'{dest_path}/{filename.split(".")[0]}.bin'
